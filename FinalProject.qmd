...
debt_to_income <- debt_to_income %>%
  filter(county == counties) # Filtering to needed counties 

# Mutate dataset to match county names of TIGRIS shapefile
debt_to_income <- debt_to_income %>%
  mutate(county = 
    if_else(county == 1, "001", as.character(county)))

debt_to_income <- debt_to_income %>%
  mutate(county = 
           if_else(county == 59, "059", as.character(county)))
    
debt_to_income <- debt_to_income %>%
  mutate(county = 
           if_else(county == 31, "031", as.character(county)))
```
