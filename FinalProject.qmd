...
debt_pca_rec <- recipe(~ ., data = debt_to_income) %>%
    step_pca(all_numeric(), num_comp = 3) %>%
    prep(data = debt_to_income) %>%
    bake(new_data = debt_to_income) %>%
    select(PC1, PC2)
...
kmeans_rec<- recipe(~ ., data = debt_to_income) %>%
...
final_fit <- debt_pca_workflow %>%
  fit(data = debt_to_income)

debt_clusters <- 
  bind_cols(debt_to_income, debt_pca_rec, cluster = final_fit %>%
