...
Loading Tigris Shapefiles from census year that matches our health and economic data. These shapefiles will come in handy when mapping health outcomes and economic data later in the project. 
...
Loading, cleaning and performing EDA for health outcomes data.
...
```
This map demonstrates that at least one of the health outcomes we selected are present in each census tracts in our dataset. Later in the project, we intend to delve further into the data to understand how common different health outcomes are in each census tract. 

### Debt-to-Income Ratio Data
Loading and cleaning debt to income ratio dataset
...
```
This hexbin plot demonstrates that the greatest concentration of loans is among the $300,000 amount in 30-40% minority census tracts. 

I am going to pick RMSE as my error metric. Since RMSE squares the errors, outliers will be penalized more. In predicting percent minority census tracts, I think that anything above a 10% difference is too much error. 
...
This geospatial visualization demonstrates the range of debt-to-income ratios by census tract in the DMV.
