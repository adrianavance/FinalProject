...
The CDC PLACES dataset maps different types of chronic diseases locally throughout the entire United States. These include estimations of the rates of various chronic conditions by census tract level. The CDC PLACES data was downloaded from the CDC website, and only data from DC, Maryland, and Virginia were manually selected. Please download the csv file provided, "PLACES.csv" to load the correct dataframe. 
...
```
FOR KAMIRYN: 'm honestly not sure I'm gonna leave this model in here, so feel free to skip over it when you're doing the write up. If I decide to keep it I will add something about it later. Thanks! However please include the second model (below) 
