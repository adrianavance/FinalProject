...
#### Loading tigris shapefiles to match our datasets for geospatial analysis
...
### Bank Location Dataset

This dataset provides the exact locations of banks in the District of Columbia. The data is available for access on Washington DC’s open data portal. The locations of the banks are provided by the Department of Insurance Securities and Banking (DISB). The dataset is updated irregularly, as needed, and provides the latitude and longitude coordinates of the banks across DC.  
...
### Visualization using banks and debt dataset
...
### Geospatial Analysis: Bank location in comparison to economic outcomes
...
### Geospatial Analysis: Bank location in comparison with rates of depression
...
Banks_CHD_400 <- max_banks_health_1 %>%
  ggplot() + 
  geom_sf(aes(fill = n)) + 
   scale_fill_gradient(
    low = "#D1E3F5",
    ...
    title = "People with CHD Within 1/4 Mile of a Bank",
  ) 
...
Banks_CHD_1600 <- max_banks_health_2 %>%
  ggplot() + 
  geom_sf(aes(fill = n)) + 
   scale_fill_gradient(
    low = "#D1E3F5",
    ...
    title = "People with CHD Within 1 Mile of a Bank",
  ) 
...
```
<<<<<<< HEAD
*** Add description about bank/depression proximity here ***
=======
*** Add description about bank/health ratio map here ***
>>>>>>> 219fcf73cde18d64b59cca8ae51beeb90a623d98

### Data Visualization: Mapping debt outcomes by census tract
...
### Data Visualization: An interactive map of debt to income ratios in the DMV
```{r echo=FALSE, warning=FALSE, message=FALSE}
...
```
*** Add description of health oucome plots here ***


#### Merging Health and Debt-to-Income dataset
Performing a spatial join of the debt-to-income dataset and the health outcomes dataset 
```{r echo=FALSE, warning=FALSE, message=FALSE}
...
### Machine Learning Model #1: Predicting percentage of minorities in a census tract using economic data (single family home loan debt and other economic observations)
```{r echo=FALSE, warning=FALSE, message=FALSE}
...
### Machine Learning Model #3: Predicting rates of depression in a census tract using economic predictors from the debt-to-income dataset
```{r echo=FALSE, warning=FALSE, message=FALSE}
...
clust_plot <- final_result %>%
  ggplot() +
  geom_point(
    mapping = aes(x = PC1, y = PC2, color = percent_minority),
    alpha = 0.5, size = 0.1
  ) +
  labs(
...
hclust_euc <- hclust(dist(dti_data), method = "ward.D2")

#plot dendrogram
plot(hclust_euc, main = "Cluster Dendogram", sub = "")
rect.hclust(hclust_euc, k = 10, border = "purple")
```
...
```
This scatterplot shows healthmeasures clustered together based upon the PCA variance of Debt-to-income variables.

```{r}
DTI_Health_clust2 <- merge(debt_to_income, Health_append, by = "CountyFIPS")
  

DTI_fun <- function(DTI_ratio, DTI_Health_clust2) {
  #pca
DTI_pca_rec <- recipe(~., DTI_Health_clust2)

DTI_pca_trans <- DTI_pca_rec |>
  step_zv(all_numeric_predictors()) |>
  step_normalize(all_numeric_predictors()) |>
  step_pca(all_numeric_predictors(), id = "pca_")

DTI_pca_estimates <- prep(DTI_pca_trans, training = DTI_Health_clust2)

DTI_Health_data <- bake(DTI_pca_estimates, DTI_Health_clust2)

#clusters
DTI_Health_spec <- k_means(
  num_clusters = 4) |>
  set_engine(
    "stats"
  )
...
#fit the final model
kmeans_pca_fit <- DTI_Health_spec |>
  fit(~., data = DTI_Health_clust2)

clusters <- extract_cluster_assignment(kmeans_pca_fit)
summary(clusters)
