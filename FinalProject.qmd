...
We obtained the census tract info using the TIGER/Line Shapefile for the District of Columbia from the U.S. Census Bureau. We installed this data through the library(tigris) command, and we manually selected for the Maryland tract, Virginia tract, and DC tract to get a picture of the whole DMV area.
...
#calculate the count of healthmeasures in new dataframe
...
```
This side by side comparison shows, in some instances, that tracts with lower DTI ratios have a higher percentage of health conditions. This may be because tracts where a majority of people are not able to leverage more debt against their income are worse off in terms of their health.
...
```
This scatterplot shows health measures clustered together based upon the PCA variance of Debt-to-income variables.However, the nonsensical nature of these graphics suggest that the cluster analysis that we fitted to these indciators was apt at predicting the relationship between these measeres. 
