...
```
This map demonstrates that at least one of the health outcomes we selected is present in each census tract in our dataset. Later in the project, we will dive further into the connection between location and health outcomes, and use economic data to further understand how health outcomes differ in each census tract. There isn't an obvious pattern among the tracts that show a concentration of specific health outcomes by specific area.
...
```
Here, we can see that most of the loans taken out are for the purposes of refinancing a mortgage, while a smaller number are for the initial purchase of homes. 
...
#scatter plot with PC1 on x-axis and PC2 on y-axis with Health measures as color
...
```
This scatterplot shows health measures clustered together based upon the PCA variance of Debt-to-income variables.However, the nonsensical nature of these graphics suggests that the cluster analysis that we fitted to these indicators was apt at predicting the relationship between these measures. 
